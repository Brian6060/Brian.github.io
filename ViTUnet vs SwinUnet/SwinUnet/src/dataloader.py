...
class KvasirSegDataset(Dataset):
    def __init__(
        ...
        self.transform = self._build_transform(train=self.train, image_size=self.image_size)

    def _build_transform(self, train: bool, image_size: int):
        ...
        return {"image": image_tf, "mask": mask_tf}
    ...
    def _apply_transform(self, image: Image.Image, mask: Image.Image) -> Tuple[Tensor, Tensor]:
        ...
        image_t = self.transform["image"](image).float()
        mask_t = self.transform["mask"](mask).float() / 255.0
        if mask_t.ndim == 2:
            mask_t = mask_t.unsqueeze(0)
        elif mask_t.ndim == 3 and mask_t.shape[0] != 1:
            mask_t = mask_t[:1]
...
if __name__ == "__main__":
    ...
    debug_dir = processed_root.parent.parent / "debug" / "swinunet_dataloader"
