...
def build_label_and_weight_map(instance_mask: np.ndarray, w_bg: float, w_fg: float,
    ...
    if len(ids) >= 2:
        ...
        dists = np.stack(dist_list, axis=0)
        dists.sort(axis=0)
    ...
    return binary_label.astype(np.float32), weight_map.astype(np.float32)
...
def export_split(split_name: str):
    ...
    for name in names:
        ...
        image_list.append(image_np[np.newaxis, :, :])
        label_list.append(label_np[np.newaxis, :, :])
        weight_list.append(weight_np[np.newaxis, :, :])
