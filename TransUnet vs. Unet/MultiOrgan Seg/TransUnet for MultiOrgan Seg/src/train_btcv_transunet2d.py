...
SAVE_ROOT = "/Users/brian/Desktop/VCL318/TransU-Net/TransUnet vs. Unet/TransUnet for MultiOrgan Seg/output/runs_btcv_transunet2d"
NUM_EPOCHS = 30
BATCH_SIZE = 8
NUM_WORKERS = 0
INPUT_SIZE = (320, 320)
...
from btcv2d_transunet_dataloader import build_btcv2d_transunet_dataloader
from transunet2d_btcv import TransUNet2D
...
def train_one_epoch(
    ...
    for _ in progress:
        ...
        batch_dice, batch_iou = multiclass_dice_iou(preds, masks, num_classes, IGNORE_BACKGROUND)

        running_loss += float(loss.item())
        running_dice += batch_dice
        running_iou += batch_iou
        num_batches += 1
        progress.set_postfix(loss=f"{float(loss.item()):.4f}", dice=f"{batch_dice:.4f}", iou=f"{batch_iou:.4f}")
...
def main() -> None:
    ...
    train_loader = build_btcv2d_transunet_dataloader(
        root_dir=data_root,
        mode="train",
    ...
    val_loader = build_btcv2d_transunet_dataloader(
        root_dir=data_root,
        mode="val",
        batch_size=BATCH_SIZE,
        num_workers=NUM_WORKERS,
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        train_loss, train_dice, train_iou = train_one_epoch(
            epoch, model, train_loader, optimizer, ce_criterion, DEVICE, num_classes
        )
        val_loss, val_dice, val_iou = validate_one_epoch(
            epoch, model, val_loader, ce_criterion, DEVICE, num_classes
        )
