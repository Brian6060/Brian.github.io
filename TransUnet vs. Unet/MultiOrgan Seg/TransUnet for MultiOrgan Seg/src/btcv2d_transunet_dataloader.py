...
class BTCV2DTransUNetDataset(Dataset):
    """2D slice dataset for BTCV multi-organ segmentation with light augmentation."""
    ...
    def __init__(
        ...
        self.use_augmentation = (self.mode == "train") if use_augmentation is None else bool(use_augmentation)

        self.records = self._load_records()
        self.foreground_records = [row for row in self.records if int(row["has_organ"]) == 1]

        if len(self.records) == 0:
            raise RuntimeError(f"No samples found for mode={self.mode} under {self.root_dir}")
    ...
    def _load_records(self) -> List[Dict]:
        ...
        target_cases = set(str(v) for v in split_info.get(split_key, []))
        filtered_df = slices_df[slices_df["case_id"].astype(str).isin(target_cases)].copy()
    ...
    def _load_pair(self, row: Dict) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if image.shape != mask.shape:
            raise ValueError(
                f"Shape mismatch for {row['slice_id']}: image.shape={image.shape}, mask.shape={mask.shape}"
            )
        return image, mask
...
def build_btcv2d_transunet_dataloader(
    root_dir: str | Path,
    mode: str,
    batch_size: int = 8,
    num_workers: int = 0,
    ...
    dataset = BTCV2DTransUNetDataset(
        root_dir=root_dir,
        mode=mode,
...
if __name__ == "__main__":
    loader = build_btcv2d_transunet_dataloader(
