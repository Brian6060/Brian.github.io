...
def find_train_pairs(data_root: Path) -> List[Tuple[str, str, Path, Path]]:
    ...
    for seq in ["01", "02"]:
        ...
        for seg_path in sorted(seg_dir.glob("man_seg*.tif")):
            match = SEG_NAME_RE.search(seg_path.name)
            if not match:
                continue
            frame = match.group(1)
...
def find_infer_images(data_root: Path) -> List[Tuple[str, str, Path]]:
    ...
    for seq in ["01", "02"]:
        ...
        for img_path in sorted(img_dir.glob("t*.tif")):
            match = IMG_NAME_RE.search(img_path.name)
            if not match:
                continue
            items.append((seq, match.group(1), img_path))
    if not items:
...
def compute_class_balance(pairs: List[Tuple[str, str, Path, Path]]) -> Dict[str, float]:
    ...
    return {
        "foreground": float(total / (2.0 * total_fg)),
        "background": float(total / (2.0 * total_bg)),
...
def process_train(data_root: Path, out_root: Path, w0: float, sigma: float) -> None:
    ...
    for seq, frame, img_path, seg_path in pairs:
        ...
        weight_arr = compute_weight_map(instance_arr, binary_arr, w_fg, w_bg, w0, sigma)
...
def main() -> None:
    ...
    if args.mode == "train":
        process_train(args.data_root, args.out_root, args.w0, args.sigma)
    else:
        process_infer(args.data_root, args.out_root)
