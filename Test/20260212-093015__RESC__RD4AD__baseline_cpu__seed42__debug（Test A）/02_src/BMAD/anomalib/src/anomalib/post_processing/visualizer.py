...
# SAFE_WANDB_LOG_PATCH
_orig_wandb_log = getattr(wandb, 'log', None)
def _safe_wandb_log(*args, **kwargs):
    try:
        if getattr(wandb, 'run', None) is not None and _orig_wandb_log is not None:
            return _orig_wandb_log(*args, **kwargs)
    except Exception:
        return None
    return None
