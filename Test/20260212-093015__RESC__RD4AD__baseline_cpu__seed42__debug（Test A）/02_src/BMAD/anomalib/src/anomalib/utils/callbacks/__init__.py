...
def get_callbacks(config: DictConfig | ListConfig) -> list[Callback]:
    ...
    # FORCE_IMAGE_VISUALIZER
    try:
        from anomalib.data import TaskType
        _task = getattr(config.dataset, 'task', 'segmentation')
        task = TaskType(_task) if not isinstance(_task, TaskType) else _task
    except Exception:
        task = getattr(config.dataset, 'task', 'segmentation')
    if hasattr(config, 'visualization') and getattr(config.visualization, 'save_images', False):
        has_img = any(getattr(cb, '__class__', None).__name__ == 'ImageVisualizerCallback' for cb in callbacks)
        if not has_img:
            callbacks.append(
                ImageVisualizerCallback(
                    task,
                    getattr(config.visualization, 'mode', 'full'),
                    getattr(config.visualization, 'image_save_path', 'images'),
                    True,
                    getattr(config.visualization, 'show_images', False),
                    getattr(config.visualization, 'log_images', False),
                    getattr(config.visualization, 'save_images', True),
                )
            )
    return callbacks
