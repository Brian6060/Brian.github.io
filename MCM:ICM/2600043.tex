...
本节对本文在写作与制图过程中使用生成式AI工具的环节进行透明披露，并说明人工核验与控制措施，确保模型结论与数据结果可追溯、可复现、可审计。

\begin{enumerate}
\item \textbf{图表视觉元素生成。}\\
\textbf{工具与版本：} OpenAI ChatGPT（GPT-5.2 Thinking）。\\
\textbf{使用位置：} 图~\ref{fig:Gen-AI Impact Evidence}（Gen-AI Impact Evidence）中间\textbf{AI应用图表的圆环（donut-ring）}视觉元素。\\
\textbf{用途：} 仅用于将不同AI模型和产品的图表进行拼接，形成圆环形状的图案。\\


\item \textbf{英文内容翻译为中文（题干与文献）。}\\
\textbf{工具与版本：} OpenAI ChatGPT（GPT-5.2 Thinking）。\\
\textbf{用途：} 将英文题干与英文参考文献的关键段落翻译为中文，帮助团队在建模初期快速统一对题意的理解、明确任务拆解，并对文献中的核心结论、假设前提与适用范围进行中文化梳理，便于内部讨论与方案选择。\\
\textbf{边界：} AI仅用于语言转换与要点梳理，不新增题目要求、不引入未在原文中出现的信息、不替代对英文原文的最终核对。所有写入论文的任务表述与文献引用均以英文原文为准，由团队逐条回查确认，避免翻译偏差导致误读题意或误引文献。

\item \textbf{\LaTeX 排版与公式对齐辅助。}\\
\textbf{工具与版本：} OpenAI ChatGPT（GPT-5.2 Thinking）。\\
\textbf{用途：} 生成或改写\LaTeX 片段（如公式对齐、表格行距/列宽、换行控制、避免溢出页面等），以提高排版稳定性与可读性。\\
\textbf{边界：} AI不参与任何计算与证明，仅提供排版层面的实现方式。所有可编译性问题与最终版式由团队在本地编译后修正确定。

\item \textbf{实现与调试建议（环境问题与制图流程）。}\\
\textbf{工具与版本：} OpenAI ChatGPT（GPT-5.2 Thinking）。\\
\textbf{用途：} 针对本地 Python 环境报错（如缺少 numpy/pandas 等依赖）提供排查步骤，并给出可复用的绘图模板代码建议。\\
\textbf{边界：} 本文所有数值结果、图表与实验输出均由团队在本地环境运行得到，并通过重复运行与参数核对完成验证。AI不直接产生任何用于结论的数值结果。

\end{enumerate}

\noindent \textbf{人工核验与合规性说明：}
团队将AI输出视为“建议稿/草案”，并执行如下控制措施：
(1) 所有核心结论仅来自模型计算与数据处理结果；
(2) 图表中出现的数值、标签与含义逐项人工复核；
(3) 任何AI生成文本在合入前均对照符号表、公式与参数设置进行一致性检查；
(4) 不使用AI伪造数据、伪造引用或替代模型验证流程。
